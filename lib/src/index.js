...
f11y.Accordion = class Accordion {
    ...
    init(){
        ...
        for (let i = 0; i < items.length; i += 1) {
            const panelElm = items[i].querySelector('[role="region"]');
            const triggerElm = items[i].querySelector("[aria-controls]");

            if(f11y.functions.isset(panelElm) === false || f11y.functions.isset(triggerElm) === false){
                console.warn('Panel or Trigger is not defined in' + items[i]);
                continue;
            }
            ...
            const itemArr = {
                ...
                panel: panelElm,
                trigger: triggerElm,
                isOpen: triggerElm.getAttribute("aria-expanded"),
            };

            triggerElm.addEventListener("click", this.toggle.bind(this, itemArr));
...
f11y.Dropdown = class Dropdown {
     ...
     * @property {string} dropdownSelector - The selector used for finding the dropdown menu
    ...
    init() {
        ...
        const dropdownId = this.triggerElm.getAttribute('aria-controls');

        /** @type {Element} */
        this.dropdownElm = this.domNode.querySelector('#' + dropdownId) ?? this.domNode.querySelector(this.options.dropdownSelector);

        if(f11y.functions.isset(this.triggerElm) === false || f11y.functions.isset(this.dropdownElm) === false){
            console.warn('Trigger or Dropdown element do not exist in passed domNode');
            return;
        }
    ...
    closeDropdown(e = null) {
        if (this.isOpen()) {
            ...
            if (this.options.awaitCloseAnimation) {
                domNode.addEventListener( 'animationend', handler );
                domNode.classList.add(animatingOpenClass, animatingCloseClass);
...
f11y.Layer = class Layer {
    ...
    openLayer (e = null) {
        ...
        const openClass = this.options.openClass;
        const animatingClass = f11y.globalSettings.animatingClass;
        const animatingOpenClass = f11y.globalSettings.animatingCloseClass;
...
f11y.TabList = class TabList {
    ...
    setActiveTabs(){
        ...
        console.log('Active Tabs ', this.activeTabs);
        console.log('Next Tab ', this.nextTab);
        console.log('Prev Tab ', this.prevTab);
