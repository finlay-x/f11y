    ...
    f11y.Toast = class Toast {
        ...
        constructor(domNode, toastTemplate, opts) {
            ...
            const DEFAULTS = {
                ...
                onDismiss: () => { },
                openClass: f11y.globalSettings.openClass,
                closeToastAttribute: 'f11y-close-tooltip',
                duration: 3000,
                oldestFirst: true,
                awaitCloseAnimation: false,
                awaitOpenAnimation: false 
        ...
        openToast(toast) {
            const toast2Insert = this.#createToast(toast);
            const insertPosition = this.options.oldestFirst ? 'afterbegin' : 'beforeend'
            this.toastContainer.insertAdjacentElement(insertPosition, toast2Insert);
            this.triggerNodes = document.querySelectorAll('[' + this.options.openTrigger + ']');
        ...
        #createToast(template) {
            ...
            if (toastTemplate.hasAttributes()) {
                for(let i = 0; i < toastTemplate.attributes.length; i++){
                    toastContainer.setAttribute(toastTemplate.attributes[i].name, toastTemplate.attributes[i].value);
                };
            }
