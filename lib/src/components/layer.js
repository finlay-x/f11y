...
class Layer {
    ...
    closeLayer(e = null){
        if(this.isOpen()){
            ...
            let activeLayers = f11y.store.activeLayers;
            const i = activeLayers.indexOf(this.id);
            activeLayers.splice(i, 1);
            f11y.store.activeLayers.length - 1;
            ...
            if(this.options.awaitCloseAnimation){
                ...
                function handler() {
                    ...
                    if(f11y.store.activeLayers.length <= 0) docBody.style.removeProperty('overflow');
                
                    domNode.removeEventListener('animationend', handler);
                }
            } else {
                domNode.classList.remove(openClass);
                f11y.store.activeLayers.length <= 0 ? docBody.style.removeProperty('overflow') : '';
            }
        }
